...
export function PersonMixin<Base extends Constructor>(Resource: Base) {
  @namespace(foaf)
  class PersonClass extends AgentMixin(Wgs.SpatialThingMixin(Resource)) implements Person {
    @property()
    currentProject: RDF.NamedNode | undefined;
    @property()
    'family_name': RDF.Literal | undefined;
...
class PersonImpl extends PersonMixin(RdfResourceImpl) {
  ...
  static readonly __mixins: Mixin[] = [PersonMixin, Wgs.SpatialThingMixin, AgentMixin];
}
